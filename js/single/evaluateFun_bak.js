let lossFun = {
    probabilityLoss: function (population10,plist) {
        ...
        return ((loss / plist.length)+math.max(lossArray))*100;
    },
    atRiskLoss: function atRiskLoss(population10) {
        ...
        for (let i = gaData.tlist.length - 1; i >= 1; i--) {
            ...
            if (totalBack > gaData.atRiskDict[curAtRiskTimeBack]) {
                loss += Math.abs(gaData.atRiskDict[curAtRiskTimeBack]-totalBack)/(gaData.atRiskDict[curAtRiskTimeBack]+1)/(gaData.tlist.length-i+1);
            }
        ...
        for (let i = 1; i < gaData.tlist.length; i++) {
            ...
            if (totalNum > gaData.riskNumberDict[curAtRiskTime]) {
                loss += Math.abs(totalNum - gaData.riskNumberDict[curAtRiskTime])/(gaData.riskNumberDict[curAtRiskTime]+1)/i;
            }
        }
        return math.min(loss,lossFun.probabilityLoss(population10));
    },
    censorLoss: function (population10) {
        ...
        return math.min(loss/gaData.plist.length,lossFun.probabilityLoss(population10));
    },
